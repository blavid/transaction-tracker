const componentSpec = {
    async run({steps, $}) {
        ...
        if (smsText.includes('Transaction Alert from First Tech Federal Credit Union')) {
            const blocks = smsText.split('***').slice(1); // each transaction starts after ***
            for (const block of blocks) {
                const match = block.match(/had a transaction of \(\$(\d+(?:\.\d+)?)\)\. Description: (.+?)\./);
                if (match) {
                    const amount = match[1];
                    let payee = match[2].trim();

                    // Normalize payee for comparison
                    const normalizedPayee = payee.toUpperCase().replace(/\s+/g, ' ').trim();

                    // Skip any credit-card payment
                    if (excludedPayees.some(ex => normalizedPayee.includes(ex))) {
                        continue; // skip this transaction completely
                    }

                    // All other First Tech transactions → record normally
